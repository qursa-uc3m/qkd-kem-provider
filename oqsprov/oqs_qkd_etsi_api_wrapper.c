...
bool qkd_open(QKD_CTX *ctx) {
    ...
    if (!ctx->source_uri) ctx->source_uri = "qkd://localhost:1234";
    if (!ctx->dest_uri) ctx->dest_uri = "qkd://localhost:5678";
    
    uint32_t status;
    //unsigned char key_stream_id[QKD_KSID_SIZE] = {0};
    
    uint32_t result = OPEN_CONNECT(
        ctx->source_uri,
        ctx->dest_uri,
        &ctx->qos,
        ctx->key_id,
    ...
    if (result == QKD_STATUS_SUCCESS || result == QKD_STATUS_PEER_DISCONNECTED) {
...
bool qkd_get_key(QKD_CTX *ctx) {
    ...
    if (result == 0 && (status == QKD_STATUS_SUCCESS || status == QKD_STATUS_PEER_DISCONNECTED)) {
        ctx->key = EVP_PKEY_new_raw_private_key(EVP_PKEY_X25519, NULL, key_buffer, QKD_KEY_SIZE);
        if (!ctx->key) {
