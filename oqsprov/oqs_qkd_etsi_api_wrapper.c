...
#ifdef ETSI_004_API
static bool open_connect_succeeded(uint32_t result, uint32_t status) {
    return result == status && (status == QKD_STATUS_SUCCESS ||
...
bool qkd_open(QKD_CTX *ctx) {
    ...
    if (open_connect_succeeded(result, status)) {
...
static unsigned char *base64_decode(const char *in, size_t *outlen) {
    ...
    *outlen = 0;
    ...
    unsigned char *out = OPENSSL_malloc(inlen);
    if (!out) {
        BIO_free_all(bmem);
        return NULL;
    }

    int decoded_length = BIO_read(bmem, out, (int)inlen);
    BIO_free_all(bmem);
...
bool qkd_get_key_with_ids(QKD_CTX *ctx) {
    ...
    if (!ctx || !ctx->slave_kme || !ctx->master_sae) {
    ...
    key_ids.key_IDs = calloc(1, sizeof(qkd_key_id_t));
    if (!key_ids.key_IDs) {
    ...
    key_ids.key_IDs[0].key_ID = OPENSSL_strdup(uuid_str);
    ...
    OPENSSL_free(key_ids.key_IDs[0].key_ID);
    free(key_ids.key_IDs);
    ...
    decoded_key = base64_decode(first_key->key, &outlen);
    if (!decoded_key) {
...
bool qkd_get_key(QKD_CTX *ctx) {
    ...
    if (!ctx->metadata.Metadata_buffer) {
        ...
        ctx->metadata.Metadata_buffer = OPENSSL_zalloc(QKD_METADATA_MAX_SIZE);
        if (!ctx->metadata.Metadata_buffer) {
    ...
    if (result == status && status == QKD_STATUS_SUCCESS) {
        ...
        OPENSSL_cleanse(key_buffer, sizeof(key_buffer));
        if (ctx->key)
            EVP_PKEY_free(ctx->key);
        ctx->key = key;
        if (!ctx->key) {
    ...
    if (!ctx->master_kme || !ctx->slave_sae) {
        QKD_DEBUG("ETSI014: Missing KME or SAE before GET_KEY call");
        return false;
    }
    ...
    req.number = 1;
    req.size = QKD_KEY_SIZE_BITS;

    QKD_DEBUG("\nETSI014: Requesting key from KME with KME=%s, SAE=%s",
              ctx->master_kme, ctx->slave_sae);

    uint32_t ret = GET_KEY(ctx->master_kme, ctx->slave_sae, &req, &container);
    QKD_DEBUG("\nETSI014: GET_KEY returned %u", ret);

    if (ret == QKD_STATUS_OK && container.key_count > 0 && container.keys) {
        qkd_key_t *first_key = &container.keys[0];
        if (!first_key->key || !first_key->key_ID) {
        ...
        if (outlen != QKD_KEY_SIZE) {
            QKD_DEBUG("ETSI014: Unexpected decoded key size: %zu", outlen);
            goto cleanup;
        }
