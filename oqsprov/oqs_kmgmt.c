...
static int oqsx_key_is_hybrid(const OQSX_KEY *oqsxk) {
    ...
    case KEY_TYPE_QKD_HYB_KEM:
        ...
        if (oqsxk->numkeys == 3 && oqsxk->classical_pkey != NULL) {
            ...
            OQS_KM_PRINTF(
                "OQSKEYMGMT: key is triple hybrid (Classical+PQC+QKD)\n");
            return 1;
        }
        break;
...
static int oqsx_get_hybrid_params(OQSX_KEY *key, OSSL_PARAM params[]) {
    ...
    if (idx_pq >= 0) {
        if (key->comp_pubkey != NULL && key->comp_pubkey[idx_pq] != NULL) {
            ...
            if (key->keytype == KEY_TYPE_QKD_HYB_KEM && idx_classic < 0) {
                 ...
                 * Total public key layout: [header (SIZE_OF_UINT32)] + [PQ
                 * pubkey] + [QKD key ID (QKD_KSID_SIZE)] Thus, PQ pubkey length
                 * = total length - header - QKD_KSID_SIZE.
                 */
        ...
        if (key->comp_privkey != NULL && key->comp_privkey[idx_pq] != NULL) {
            ...
            if (key->keytype == KEY_TYPE_QKD_HYB_KEM && idx_classic < 0) {
                /* For the private key layout: [header (SIZE_OF_UINT32)] + [PQ
                 * privkey] + [QKD key (QKD_KEY_SIZE)] Thus, PQ privkey length =
                 * total length - header - QKD_KEY_SIZE.
                 */
                pq_privkey_len =
                    key->privkeylen - SIZE_OF_UINT32 - QKD_KEY_SIZE;
            } else {
                pq_privkey_len =
    ...
    if (idx_pq >= 0) {
        if ((p = OSSL_PARAM_locate(params, OQS_HYBRID_PKEY_PARAM_PQ_PUB_KEY)) !=
            ...
            return -1;
        if ((p = OSSL_PARAM_locate(
                 params, OQS_HYBRID_PKEY_PARAM_PQ_PRIV_KEY)) != NULL &&
            !OSSL_PARAM_set_octet_string(p, pq_privkey, pq_privkey_len))
            return -1;
    }
    if (idx_qkd >= 0) {
        if ((p = OSSL_PARAM_locate(
                 params, OQS_HYBRID_PKEY_PARAM_QKD_PUB_KEY)) != NULL &&
...
MAKE_KEM_QKD_KEYMGMT_FUNCTIONS(frodo640shake, OQS_KEM_alg_frodokem_640_shake,
                               128)
MAKE_KEM_QKD_KEYMGMT_FUNCTIONS(frodo976aes, OQS_KEM_alg_frodokem_976_aes, 192)
MAKE_KEM_QKD_KEYMGMT_FUNCTIONS(frodo976shake, OQS_KEM_alg_frodokem_976_shake,
                               192)
MAKE_KEM_QKD_KEYMGMT_FUNCTIONS(frodo1344aes, OQS_KEM_alg_frodokem_1344_aes, 256)
MAKE_KEM_QKD_KEYMGMT_FUNCTIONS(frodo1344shake, OQS_KEM_alg_frodokem_1344_shake,
                               256)
